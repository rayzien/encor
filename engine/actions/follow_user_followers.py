"""
follow_user_followers: Scrapes the follower list of a competitor or target account and follows them.
...
logger = logging.getLogger("encor.actions.follow_user_followers")

async def follow_user_followers(page: Page, rate_limiter: RateLimiter, usernames: List[str], amount: int = 10, randomize: bool = True, interact: bool = False, logger_callback=None) -> Dict[str, Any]:
    ...
    for target_user in usernames:
        ...
        msg = f"Fetching followers of @{target_user}"
        logger.info(msg)
        if logger_callback: logger_callback(msg)

        follower_list = await InstagramScraper.get_user_followers(page, target_user, amount=per_target * 2)
        ...
        for user in follower_list:
            ...
            try:
                ...
                if follow_btn:
                    ...
                    msg = f"Followed @{user} (follower of @{target_user})"
                    logger.info(msg)
                    if logger_callback: logger_callback(msg)

                    if interact:
                        # Like 1 recent post if available
